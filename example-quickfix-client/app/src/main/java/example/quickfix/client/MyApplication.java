...
private static String fixPretty(String s) {
    if (s == null) return "";
    return s
        .replace('\u0001', '|')   // SOH 0x01
        .replace('\u0000', '|');  // NUL 0x00 (if any)
}

 public String drainMessagesAsPrettyString() {
    StringBuilder out = new StringBuilder();
    synchronized (messageBuffer) {
        for (quickfix.Message m : messageBuffer) {
            out.append(fixPretty(m.toString())).append('\n');
        }
        messageBuffer.clear();
    }
    synchronized (sessionBuffer) {
        for (quickfix.SessionID s : sessionBuffer) {
            out.append(s).append('\n');
        }
        sessionBuffer.clear();
    }
